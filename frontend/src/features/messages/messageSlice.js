...
const initialState = {
	...
	isSuccess: false,
	isLoading: false,
	message: '', // just the corresp. message, NOT a message in terms of app purpose
}
...
export const createMessage = createAsyncThunk('messages/create', async (messageData, thunkAPI) => {
	try {
		...
		return messageService.createMessage(messageData, token)
	} catch (error) {
		const message =
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(deleteMessage.fulfilled, (state, action) => {
				state.isLoading = false
				state.isSuccess = true
				state.messages = state.messages.filter((message) => message._id !== action.payload.id)
			})
